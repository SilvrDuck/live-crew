...
from live_crew.config import LiveCrewConfig, load_config
...
class TestConfigIntegration:
    ...
    def test_config_affects_timeslice_constant(self):
        """Test that configuration can override SLICE_MS constant."""
        from live_crew.timeslice import SLICE_MS

        # Default SLICE_MS should be 500
        assert SLICE_MS == 500

        # Test that we can create config with different slice_ms
        config = LiveCrewConfig(slice_ms=250)
        assert config.slice_ms == 250
        assert config.slice_ms != SLICE_MS  # Config value differs from constant
